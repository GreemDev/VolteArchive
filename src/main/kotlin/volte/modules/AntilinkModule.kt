...
class AntilinkModule : ListenerAdapter() {
    ...
    override fun onGuildMessageReceived(event: GuildMessageReceivedEvent) {
        if (event.guild.getData().getAntilink().not()) return
        if (DiscordUtil.isOperator(event.member!!).not()) return
        ...
        if (matcher.matches()) {
            ...
            event.guild.retrieveMember(event.author) then { member ->
                event.channel.sendMessage(DiscordUtil.createDefaultEmbed("${member.asMention}, don't send invite links here!", member).build()).queue()
            }
        }
    }
